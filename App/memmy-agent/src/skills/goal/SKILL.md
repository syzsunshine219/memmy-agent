...
`long_task` is a lightweight marker. Calling it tells memmy: "this thread has a sustained objective; keep that objective visible across turns and surface it in the UI."
...
6. **`uiSummary`** — Short label for sidebars/logs; keep **non-load-bearing** (no secret requirements only in the summary).
